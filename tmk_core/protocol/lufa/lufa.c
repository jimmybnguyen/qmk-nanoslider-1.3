...
void Init_ADC1(void){
	...
	ADMUX = ( 1 << REFS0);   // External reference voltage on Avcc with capacitor
	ADCSRB |= ( 1 << MUX5); //ADC 8
	ADCSRA =(1 << ADPS2)| (1 << ADPS1) |(1 << ADPS0); // prescale /128 consider 8MHZ clock
	ADCSRA |= ( 1 << ADEN );  // Enable ADC
}

uint16_t AMP_calculator(void){
	uint16_t sample_f;
...

{
	...
	uint8_t MIDIVelocity  = 127 - AMP_calculator();
  ...
  uint8_t Channel = 2;
...

	{
		...
		MIDI_Device_SendEventPacket(&USB_MIDI_Interface, &MIDIEvent);
